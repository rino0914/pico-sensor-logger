...
- SCD40의 CO₂·온도·습도 측정
- 최근 10분 측정값을 JSON으로 받아 브라우저에서 그리는 대화형 SVG 그래프
...
- 측정 시작·중지 및 데이터 초기화
- 최대 20분 자동 측정 종료와 CSV 240행 안전 상한
- 측정값 CSV 저장 및 다운로드
...
7. 측정은 설정된 시간(기본 20분)이 되면 자동으로 완료됩니다. 일찍 끝내려면 `측정 중지`를 누릅니다.
8. 완료된 CSV를 내려받은 뒤 `데이터 초기화`를 눌러야 다음 실험을 시작할 수 있습니다.
...
`measurement.max_duration_minutes`는 한 번의 실험 시간을 정수 분 단위로 설정하며 `1`부터 `20`까지 허용합니다. 제한 시간이 되면 측정과 상태 LED가 자동으로 꺼지고 대시보드에 완료 상태가 표시됩니다. 약 5초 간격을 기준으로 20분 CSV는 최대 240개 측정행으로 제한됩니다. 재부팅하더라도 기존 CSV 행 수를 확인하므로 파일이 제한을 넘어 계속 커지지 않습니다. 측정 완료 후에는 CSV를 내려받고 `데이터 초기화`를 눌러야 새 측정을 시작할 수 있습니다. 대시보드 그래프는 메모리 절약을 위해 CSV 전체와 별개로 최근 10분만 표시합니다.

AP 모드는 비밀번호 없이 개방형으로 동작합니다. 주변 사용자는 누구나 접속하고 측정을 제어할 수 있으므로 통제된 실험 환경에서 사용해야 합니다.

### AP 채널 설정 가이드

Pico W는 2.4GHz Wi-Fi만 사용합니다. 이 프로젝트의 `wifi.ap.channel`에는 `1`부터 `11`까지의 정수만 설정할 수 있으며, 값을 생략하면 채널 `1`을 사용합니다. 일부 국가에서는 채널 12와 13도 허용되지만 Pico W 펌웨어의 전 세계 공통 안전 설정과 스마트폰 호환성을 고려하여 이 프로젝트에서는 사용하지 않습니다.

2.4GHz 채널 번호는 서로 다른 주파수의 중심점을 나타내지만 인접 채널의 실제 주파수 범위는 겹칩니다. 따라서 기기 네 대에 `1`, `2`, `3`, `4`처럼 연속된 채널을 지정하면 간섭을 줄이기보다 늘릴 수 있습니다. 일반적으로 서로 겹치지 않는 채널 `1`, `6`, `11`을 우선 사용합니다.

| 동시에 실행하는 Pico 수 | 권장 채널 배치 |
|---:|---|
| 1대 | 주변에서 가장 덜 혼잡한 `1`, `6`, `11` 중 하나 |
| 2대 | `1`, `6`, `11` 중 서로 다른 두 개 |
| 3대 | `1`, `6`, `11` |
| 4대 | `1`, `6`, `11`을 사용하고, 네 번째는 가장 덜 혼잡한 채널을 재사용 |

채널은 SSID처럼 기기마다 반드시 고유할 필요가 없습니다. 같은 채널의 AP는 송신 순서를 나누어 사용하므로 속도와 응답성이 낮아질 수 있지만, 인접한 겹침 채널을 사용하는 것보다 예측하기 쉽습니다. 네 번째 기기는 같은 채널을 쓰는 다른 Pico와 가능한 한 멀리 배치합니다.

기기별 `config.json` 예시는 다음과 같습니다. 소스코드는 동일하게 배포하고 `channel` 값만 `1`, `6`, `11`로 변경합니다.
...
부팅 로그의 `WiFi channel (requested)`와 `WiFi channel (active)`가 같은지 확인합니다. 실제 교실에서는 먼저 Pico를 모두 끈 상태에서 스마트폰의 Wi-Fi 분석 도구로 주변 AP가 사용하는 채널을 확인하고, `1`, `6`, `11` 중 가장 덜 혼잡한 채널부터 배정하는 것이 좋습니다. Pico W 안테나 주변에는 금속이나 다른 Pico를 바짝 붙이지 않습니다. MicroPython의 채널 설정 방식은 [WLAN 설정 문서](https://docs.micropython.org/en/latest/library/network.WLAN.html)를 참고합니다.

AP 시작 시 활성 상태, 실제 적용 채널, IPv4 설정과 MAC 주소를 검증합니다. 실행 중에도 2초마다 이 상태를 확인하며 불일치나 비활성화가 감지되면 오류를 발생시켜 애플리케이션을 종료합니다. AP는 자기 무선 신호의 RSSI나 외부 간섭을 직접 측정할 수 없으므로 신호 품질은 연결된 스마트폰이나 별도의 Wi-Fi 분석 도구에서 확인해야 합니다.

`diagnostics.wifi_trace`를 `true`로 설정하면 AP 활성화 직후 CYW43 저수준 추적을 켭니다. DHCP와 Wi-Fi 이벤트 조사에 사용할 수 있지만 로그가 매우 많고 네트워크 데이터가 포함될 수 있으므로 장애를 재현할 때만 사용하고 평상시에는 `false`로 둡니다. `false`로 시작하면 soft reboot 전에 활성화됐던 추적도 명시적으로 해제합니다. 펌웨어가 추적 기능을 지원하지 않으면 경고만 출력하고 계속 실행합니다.
...
- 비교 그래프는 약 5초 간격의 측정값을 최대 120개, 약 10분간 유지하며 브라우저에서 5초마다 갱신합니다.
- 한 번의 측정은 설정에 따라 최대 20분이며 CSV는 최대 240개 측정행을 저장합니다.
