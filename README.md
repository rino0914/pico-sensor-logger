...
`diagnostics.wifi_trace`를 `true`로 설정하면 AP 활성화 직후 CYW43 저수준 추적을 켭니다. DHCP와 Wi-Fi 이벤트 조사에 사용할 수 있지만 로그가 매우 많고 네트워크 데이터가 포함될 수 있으므로 장애를 재현할 때만 사용하고 평상시에는 `false`로 둡니다. 펌웨어가 추적 기능을 지원하지 않으면 경고만 출력하고 계속 실행합니다.
